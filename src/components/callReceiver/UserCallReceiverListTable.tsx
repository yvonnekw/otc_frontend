...
const UserCallReceiverListTable: React.FC<Props> = ({username}) => {
    ...
    useEffect(() => {
        const fetchCallReceivers = async () => {
            setLoading(true);
            try {
                const data: Receiver[] = await getUserCallReceiverList(username);
...
const UserCallReceiverListTable: React.FC<Props> = ({ username }) => {
    ...
    const columns: GridColDef[] = [
        { field: 'callReceiverId', headerName: 'callReceiver ID', width: 150 },
        { field: 'fullName', headerName: 'Full Name', width: 180 },
        { field: 'telephone', headerName: 'Telephone', width: 180 },
        { field: 'relationship', headerName: 'Relationship', width: 200 },
    ...
    useEffect(() => {
        const fetchCallReceivers = async () => {
            setLoading(true);
            try {
                const data = await getUserCallReceiverList(username);

                const callReceiversWithId = data.map((callReceiver) => ({
                    ...callReceiver,
                    id: parseInt(callReceiver.callReceiverId)
                }));
                setUserCallReceivers(callReceiversWithId);
                setFilteredUserCallReceivers(callReceiversWithId);
                setLoading(false);
    ...
    if (loading) {
        return (
            <Container data-testid="user-call-receiver-list-table-loading" sx={{ mt: 5, display: 'flex', justifyContent: 'center' }}>
                <CircularProgress />
            </Container>
        );
    }
    ...
    return (
        <Container data-testid="user-call-receiver-list-table" sx={{ mt: 5 }}>
            ...
            <TextField
                ...
                data-testid="user-call-receiver-search-field"
            />
            <span data-testid="user--call-receiver-table-title" style={{ display: 'none' }}>
                User Call Receiver List Table
            </span>
            <div style={{ height: 600, width: '100%' }}>
                <DataGrid
                    rows={filteredUserCallReceivers}
